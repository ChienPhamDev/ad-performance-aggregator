...
class Benchmark {
  ...
  stop(label = 'default') {
    ...
    if (!timer) {
      throw new Error(`Benchmark with label "${label}" has not been started.`);
    }
    ...
    const memUsed = {
      rss: ((endMemory.rss - timer.startMemory.rss) / (1024 * 1024)).toFixed(2),
      heapTotal: ((endMemory.heapTotal - timer.startMemory.heapTotal) / (1024 * 1024)).toFixed(2),
      heapUsed: ((endMemory.heapUsed - timer.startMemory.heapUsed) / (1024 * 1024)).toFixed(2),
    };
