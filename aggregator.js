...
async function run() {
  ...
  const { input, output, parallel } = program.opts();
  ...
  try {
    ...
    const campaigns = await aggregator.process({ parallel });
    ...
    csvWriter.save('top10_cpa.csv', reportService.getTop10ByLowestCpa(campaigns));
